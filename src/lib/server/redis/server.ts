...
//@ts-ignore
import type { Rooms, WagerTx, Chat } from '../state.js';
...
.then(async () => {
    ...
    app.get('/makeballroom', async (req, res) => {
        ...
        let hasRoom
        let jBallRooms:any = await client.get('BALLROOMS')
        ballRooms = JSON.parse(jBallRooms)
        // ballRooms = await client.get('BALLROOMS').then(rooms => {
        //     if (rooms.length && rooms != null) ballRooms = JSON.parse(rooms)
        //     console.log(ballRooms)
        // })


        if (ballRooms.length) hasRoom = ballRooms?.find(room => room.players.includes(user) && room.game == '8 Ball')
        console.log(hasRoom)
...
async function getRoomsOnStartup() { // Doesn't require 'ENDEDROOMS' key as this is only grabbing
    if (await client.get('ROOMS') != null) {
        let jRooms:any =  await client.get('ROOMS')
